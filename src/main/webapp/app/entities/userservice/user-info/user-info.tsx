...
export const UserInfo = () => {
  ...
  const userInfoList = useAppSelector(state => state.gateway.userInfo.entities);
  const loading = useAppSelector(state => state.gateway.userInfo.loading);
  const totalItems = useAppSelector(state => state.gateway.userInfo.totalItems);
  ...
  return (
    <div>
      <h2 id="user-info-heading" data-cy="UserInfoHeading">
        <Translate contentKey="gatewayApp.userserviceUserInfo.home.title">User Infos</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="gatewayApp.userserviceUserInfo.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/user-info/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="gatewayApp.userserviceUserInfo.home.createLabel">Create new User Info</Translate>
          </Link>
        </div>
      </h2>
        ...
        {userInfoList && userInfoList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('alias')}>
                  <Translate contentKey="gatewayApp.userserviceUserInfo.alias">Alias</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('alias')} />
                </th>
            ...
            <tbody>
              {userInfoList.map((userInfo, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`gatewayApp.GenderType.${userInfo.gender}`} />
                  </td>
                  ...
                  <td>{userInfo.createdAt ? <TextFormat type="date" value={userInfo.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{userInfo.updatedAt ? <TextFormat type="date" value={userInfo.updatedAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/user-info/${userInfo.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/user-info/${userInfo.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gatewayApp.userserviceUserInfo.home.notFound">No User Infos found</Translate>
            </div>
          )
        )}
      </div>
