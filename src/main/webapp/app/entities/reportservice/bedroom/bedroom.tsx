...
import { getEntities } from './bedroom.reducer';
...
export const Bedroom = () => {
  ...
  const bedroomList = useAppSelector(state => state.gateway.bedroom.entities);
  const loading = useAppSelector(state => state.gateway.bedroom.loading);
  const totalItems = useAppSelector(state => state.gateway.bedroom.totalItems);
  ...
  return (
    <div>
      <h2 id="bedroom-heading" data-cy="BedroomHeading">
        <Translate contentKey="gatewayApp.reportserviceBedroom.home.title">Bedrooms</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="gatewayApp.reportserviceBedroom.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/bedroom/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="gatewayApp.reportserviceBedroom.home.createLabel">Create new Bedroom</Translate>
          </Link>
        </div>
      </h2>
        ...
        {bedroomList && bedroomList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  <Translate contentKey="gatewayApp.reportserviceBedroom.report">Report</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {bedroomList.map((bedroom, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/bedroom/${bedroom.id}`} color="link" size="sm">
                      {bedroom.id}
                    </Button>
                  </td>
                  ...
                  <td>
                    <Translate contentKey={`gatewayApp.QualityStateType.${bedroom.conditionLevel}`} />
                  </td>
                  ...
                  <td>{bedroom.report ? <Link to={`/report/${bedroom.report.id}`}>{bedroom.report.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/bedroom/${bedroom.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/bedroom/${bedroom.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/bedroom/${bedroom.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gatewayApp.reportserviceBedroom.home.notFound">No Bedrooms found</Translate>
            </div>
          )
        )}
      </div>
